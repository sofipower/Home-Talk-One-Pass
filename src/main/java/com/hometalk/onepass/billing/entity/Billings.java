 ...
 *  - C5: NOT NULL(household_id, billing_month, total_amount, due_date) - 핵심 정보 누락 방지
 *
 * 연관 관계:
...
public class Billings extends BaseEntity {
     ...
     *
     * 화면 적용:
     * - 입주민: 주황색(UNPAID) / 초록색(PAID) 배지, 미납 배너 노출 여부
     * - 관리자: 미납만 보기 필터, 납부완료 처리 버튼 활성화 기준
     */
    ...
    /**
     * 납부 완료 처리
     * - 관리자 납부완료 처리 confirm 시 호출
     * - BillingLogs에 STATUS_CHANGE 기록은 서비스 레이어에서 처리
     */
    ...
    /**
     * 고지서 업로드(UPSERT) UPDATE 케이스 - 기존 월 데이터 갱신
     * - total_amount, due_date를 최신 엑셀 데이터로 덮어씀
     * - status는 유지 (이미 PAID 처리된 건은 변경하지 않음)
     */
